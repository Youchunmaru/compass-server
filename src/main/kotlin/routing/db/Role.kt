...
fun Application.configureRoleRouting(dbService: DBService) {
    val service = dbService.roleService
    routing {
        authenticate("auth-jwt") {
            route("/roles") {
                post("/create"){
                    doAuthOperation("${DBPermissions.ROLES}.${CRUDPermissions.CREATE}", call, dbService) {
                ...
                get("/{id}") {
                    doAuthOperation("${DBPermissions.ROLES}.${CRUDPermissions.READ}", call, dbService) {
                ...
                put("/{id}") {
                    doAuthOperation("${DBPermissions.ROLES}.${CRUDPermissions.UPDATE}", call, dbService) {
                        val id = call.parameters["id"]?.toInt() ?: throw IllegalArgumentException("Invalid ID")
                        val model = service.routingReceive(call)
                ...
                delete ("/{id}"){
                    doAuthOperation("${DBPermissions.ROLES}.${CRUDPermissions.DELETE}", call, dbService) {
                        val id = call.parameters["id"]?.toInt() ?: throw IllegalArgumentException("Invalid ID")
                        service.delete(id)
                        call.respond(HttpStatusCode.OK)
                    }
