...
fun Route.configureGroupRouting(dbService: DBService) {
    val service = dbService.groupService
    route("/groups") {
        post("/create"){
            doAuthOperation("${DBPermissions.GROUPS}.${CRUDPermissions.CREATE}", call, dbService) {
                create(service, call)
            }
        }
        get("/{id}") {
        ...
        put("/{id}") {
            doAuthOperation("${DBPermissions.GROUPS}.${CRUDPermissions.UPDATE}", call, dbService) {
                update(service, call)
            }
        }
        delete ("/{id}"){
            doAuthOperation("${DBPermissions.GROUPS}.${CRUDPermissions.DELETE}", call, dbService) {
                delete(service, call)
            }
        }
        get ("/all"){
